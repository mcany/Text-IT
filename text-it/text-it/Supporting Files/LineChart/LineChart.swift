...
class LineChart: CALayer {
    ...
    override func layoutSublayers() {
        ...
        for yAxis in yAxes {
            ...
            let minYValue = datasetsForAxis.reduce(CGFloat.max, combine: { min($0, minElement($1.data)) })
            let maxYValue = datasetsForAxis.reduce(CGFloat.min, combine: { max($0, maxElement($1.data)) })
            ...
            let maxTickWidth: CGFloat = yAxis.ticks.reduce(0.0, combine: { max($0, $1.width) })
